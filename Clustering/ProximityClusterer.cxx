#ifndef LARLITE_PROXIMITYCLUSTERER_CXX
#define LARLITE_PROXIMITYCLUSTERER_CXX
...
namespace larlite {
  ...
  ProximityClusterer::ProximityClusterer(){

    _name        = "ProximityClusterer";
    _fout        = 0;
    _verbose     = false;
  ...
  bool ProximityClusterer::analyze(storage_manager* storage) {
  ...
  bool ProximityClusterer::finalize() {
  ...
  bool ProximityClusterer::HitsCompatible(const hit& h1, const hit& h2){

    if (h1.WireID().Plane != h2.WireID().Plane)
      return false;

    double dt =  TimeOverlap(h1,h2);
  ...
  double ProximityClusterer::TimeOverlap(const larlite::hit& h1,
					 const larlite::hit& h2) const
  {
    bool overlap = false;
    
    double dmin = _cellSize * _cellSize * 4;
  ...
  void ProximityClusterer::MakeHitMap(const event_hit* hitlist, int plane){
