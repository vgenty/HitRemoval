#ifndef LARLITE_ISOLATEDCLUSTERER_CXX
#define LARLITE_ISOLATEDCLUSTERER_CXX

#include "IsolatedClusterer.h"
...
namespace larlite {
  ...
  IsolatedClusterer::IsolatedClusterer(){

    _name        = "IsolatedClusterer";
    _fout        = 0;
    _verbose     = false;
  ...
  bool IsolatedClusterer::initialize() {
  ...
  bool IsolatedClusterer::analyze(storage_manager* storage) {
	...
	// create list of clusters for this cell
	// we ultimately want only cells with a single cluster in them
	std::map<size_t, size_t> CellclusterMap;
	// cluster index
	size_t CellclusterIdx = 0;
	...
	if (cellhits.size() != neighborhits.size() ) continue;
  ...
  bool IsolatedClusterer::finalize() {

    return true;
  }

  // get all hits from neighboring cells
  void IsolatedClusterer::getNeighboringHits(const std::pair<int,int>& pair, std::vector<size_t>& hitIndices){
  ...
  bool IsolatedClusterer::HitsCompatible(const hit& h1, const hit& h2){
  ...
  bool IsolatedClusterer::TimeOverlap(const larlite::hit& h1,
  ...
  void IsolatedClusterer::MakeHitMap(const event_hit* hitlist, int plane){
