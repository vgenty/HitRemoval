#ifndef LARLITE_SIMPLECLUSTERERMULTIVERTEX_CXX
#define LARLITE_SIMPLECLUSTERERMULTIVERTEX_CXX

#include "SimpleClustererMultiVertex.h"
...
namespace larlite {
  ...
  SimpleClustererMultiVertex::SimpleClustererMultiVertex(){

    _name        = "SimpleClustererMultiVertex";
    _fout        = 0;
    _verbose     = false;
  ...
  bool SimpleClustererMultiVertex::initialize() {
  ...
  bool SimpleClustererMultiVertex::analyze(storage_manager* storage) {
    ...
    if ( (!_vtxProducer.empty()) and (!evt_vtx) ){
    ...
    if (!_vtxProducer.empty()){

      auto geoH = larutil::GeometryHelper::GetME();
	
      _vtx_w_cm_vv.resize(evt_vtx->size());
      _vtx_t_cm_vv.resize(evt_vtx->size());
      
      for(size_t vtxid=0; vtxid < evt_vtx->size(); ++vtxid) {
	const auto& vtx = evt_vtx->at(vtxid);
	auto& vtx_w_cm_v = _vtx_w_cm_vv[vtxid];
	auto& vtx_t_cm_v = _vtx_t_cm_vv[vtxid];
	std::vector<double> xyz = {vtx.X(), vtx.Y(), vtx.Z()};
	for (size_t pl = 0; pl < 3; pl++){
	  auto const& pt = geoH->Point_3Dto2D(xyz,pl);
	  vtx_w_cm_v[pl] = pt.w;
	  vtx_t_cm_v[pl] = pt.t + 800 * _time2cm;
	}
      }
    }
  ...
  bool SimpleClustererMultiVertex::finalize() {

    return true;
  }

  // get all hits from neighboring cells
  void SimpleClustererMultiVertex::getNeighboringHits(const std::pair<int,int>& pair, std::vector<size_t>& hitIndices){
  ...
  bool SimpleClustererMultiVertex::HitsCompatible(const hit& h1, const hit& h2){
  ...
  bool SimpleClustererMultiVertex::TimeOverlap(const larlite::hit& h1,
  ...
  void SimpleClustererMultiVertex::MakeHitMap(const event_hit* hitlist, int plane){
	...
	bool skip = true;
	for(size_t vtxid=0; vtxid<_vtx_w_cm_vv.size(); ++vtxid) {
	  double wcm = fabs(t - _vtx_t_cm_vv[vtxid][plane]);
	  double tcm = fabs(w - _vtx_w_cm_vv[vtxid][plane]);
	  double d = sqrt( ( (t - _vtx_t_cm_vv[vtxid][plane]) * (t - _vtx_t_cm_vv[vtxid][plane]) ) +
			   ( (w - _vtx_w_cm_vv[vtxid][plane]) * (w - _vtx_w_cm_vv[vtxid][plane]) ) );
	  if (d < _vtx_radius) {
	    break;
	  }
	...
	} // end this vertex check

	// its outside of range
	if (skip) continue;
      }
